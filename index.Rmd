...
```{r echo=FALSE}
dfClaimTransactions %>% 
  select(-PolicyExpirationDate, -Exposure, -PolicyholderID, -NumberOfPayments) %>% 
  head(5) %>% 
  kable()
```
...
```
linkRatio_1_2 <- lm(CumulativePaid)
```
...
The source code for these slides may be found here: [where is this?]()

## Some References

<!-- Change the notes section to a bibliography -->
...
* Read this: http://www.casact.org/pubs/forum/17spforumv2/01_Boor.pdf
